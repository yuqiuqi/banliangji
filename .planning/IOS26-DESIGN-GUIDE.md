...
| R2 | **色彩节制（Restrained Color）** 🍎 OFF | 控件与导航的饱和度要让位给内容，避免「彩色 Chrome」喧宾夺主。 |
| R3 | **同心圆角（Concentric Corners）** 🍎 OFF | 子元素圆角 = 父容器圆角 − 内边距；破坏同心 → 视觉「错位」。 |
| R4 | **动效是物理，而非时序** 📝 AUTH | 所有状态切换优先走弹簧（Spring），`timing` 只用于 opacity / Scrim 类辅助。 |
...
### 3.1 三种变体（Apple 官方） 🍎 OFF 🛠️ RN-T1 📦 expo-blur
...
### 3.3 RN 工程近似（本仓库） 🛠️ RN-T1 📦 expo-blur
...
### 3.5 八大核心视觉效果总览（Signature Effects） 🍎 OFF（E1–E8 均为 Apple 官方描述） 🛠️ RN-T1/T2/T3 分级（见下表）
...
### 4.1 色彩哲学：**内容浸透控件（Content Infuses Controls）** 🍎 OFF
...
### 4.2 强调色使用清单（Accent Reserved List） 📝 AUTH
...
- Clear 变体目前 Expo 无原生支持 —— 列入 Accepted Deviation（§21 D14）
...
### 5.2 字阶（Type Scale）— 强制 🍎 OFF（对齐 Apple Typography） 🛠️ RN-T1（`iosType` 已在 `src/theme/typography.ts`）
...
### 6.2 间距 Token 映射（本仓库） 📝 AUTH 🛠️ RN-T1
...
### 6.3 圆角规模（iOS 26：「更圆更松」） 🍎 OFF 🛠️ RN-T1
...
### 6.4 同心圆角（Concentric Corners） 🍎 OFF 🛠️ RN-T1
...
### 7.1 iOS 26 阴影语言：**极轻、大模糊半径** 📝 AUTH

iOS 26 的阴影**不是**为了「立体」，而是为了**锚定层级**。视觉特征：**低 opacity × 大 blur × 小 offset**。

### 7.2 阴影 Token（本仓库已固化） 📝 AUTH 🛠️ RN-T1
...
### 8.2 Tab Bar（iOS 26 浮动 Tab） 🍎 OFF 🛠️ RN-T2
...
### 8.3 Toolbar（原 Navigation Bar） 🍎 OFF
...
### 8.4 内容边缘效果（Scroll Edge Effect） 🍎 OFF 🛠️ RN-T2 📦 react-native-reanimated
...
- ❌ 在系统原生控件（`RN Button` / `Pressable`）上写死 `height: 44`（忽略 Dynamic Type）
...
### 9.2 分段控件（Segmented Control） 📝 AUTH 🛠️ RN-T1 📦 react-native-reanimated
...
### 9.3 列表行（List Row） 🍎 OFF（HIG 最小 44pt） 📝 AUTH（按压态本仓库规范）
...
### 9.5 FAB（Floating Action Button） 📝 AUTH（本仓库模式，非 Apple 原生控件）
...
### 11.1 全局弹簧表（**唯一来源** `src/theme/motion.ts`） 📝 AUTH 🛠️ RN-T1 📦 react-native-reanimated

> *当前仓库的 `src/theme/motion.ts` 已被 Phase 15 revert 清除。本文声明其为**未来必恢复资产**，Phase 17 第一件事即重建此文件。*
...
### 11.3 按压弹簧（Spring Press） — **P0** 📝 AUTH 🛠️ RN-T1 📦 react-native-reanimated
...
### 11.9 Reduce Motion 降级（全局） 🍎 OFF 🛠️ RN-T1
...
### 12.1 手势库 🛠️ RN-T1 📦 react-native-gesture-handler
...
### 13.1 iOS 触感三大类 🍎 OFF 🛠️ RN-T1 📦 expo-haptics
...
### 16.1 必做项（不做即缺陷） 🍎 OFF（A1/A2/A5/A6/A7/A9 为 Apple 官方要求） 📝 AUTH（A3/A4/A8 为本仓库落地规范）
...
### 17.1 帧率目标 📝 AUTH 🛠️ RN-T1 📦 react-native-reanimated
...
> **v1.2 变更：** 不再重复内容 —— 各偏差的详细 SwiftUI 原生行为、RN 近似方案、判定条件，均写在 §3.5–§3.15 各效果子节内的「RN 工程近似」段。本表作为**跨章节索引**，任何相位 `VERIFICATION.md` 登记偏差时直接引用 ID + 对应章节。

| ID | 简要偏差 | 详细章节 | 判定 |
|----|---------|---------|------|
| **D1** | `.glassEffect(.regular)` 真实 lensing → `BlurView + overlay + hairline` | §3.6 E1 | Accepted |
| **D2** | `GlassEffectContainer` 共享采样 + morphing → 多 `BlurView` 独立 | §3.10 G1 | Accepted（建议使用 Container 模式） |
