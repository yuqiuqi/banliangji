...
> **边界：** 本阶段负责 **全局 Clay 层次、列表行、卡片、顶区、可感知按压与轻过渡**。**图表 Tab 内** 柱/条/图例/周月年切换的 图表精致化向表现归 **Phase 7（REF-03）**；Phase 6 仅可改 `ChartScreen` 的 **分段器、卡片容器、标题区、外壳列表行**，**禁止** 改动 `chartAggregate.ts` 及区间语义。
