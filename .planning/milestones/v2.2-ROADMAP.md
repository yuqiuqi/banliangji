...
在已实现的 Expo + TS + SQLite 基线上，按「数据可信 → 功能验收 → 图表一致 → 质量门禁」顺序收敛到可发布的 **v1.0**；随后以 **milestone v2.0** 扩展 **预算、资产、图表表现与全局 UI**，全部纳入 **半两记** 统一体验（保持离线优先、技术栈不偏离 Expo / RN）。优先保证离线闭环与类型/静态检查。
...
- **Phase 1: 数据层与基线验证** — SQLite 契约、回归检查、关键路径文档化   (completed 2026-04-21)
- **Phase 2: 核心用户流程 UAT** — 明细/记一笔/详情/编辑/删除/日历走查与缺陷修复   (completed 2026-04-21)
- **Phase 3: 图表与分析一致性** — 周/月/年支出聚合与分类展示对齐产品预期   (completed 2026-04-21)
- **Phase 4: 质量门禁与发布准备** — typecheck/lint/手工 UAT 清单与版本说明   (completed 2026-04-21)
- **Phase 5: 预算与资产管家 MVP（v2）** — REF-01 / REF-02；SQLite 扩展与 Tab/导航   (completed 2026-04-21)
- **Phase 6: 全局 UI 与动效精研（v2）** — REF-04；Clay+ 层级、转场与关键屏改版   (completed 2026-04-22)
- **Phase 7: 图表 高规格表现层（v2）** — REF-03；依赖 Phase 3 数据正确   (completed 2026-04-22)
- **Phase 8: 账单流 · 查账 · 我的与 Tesla 收敛（v2.1）** — V21-01 / V21-02 / V21-03 / V21Q-01   (completed 2026-04-22)
- **Phase 9: iOS & iPadOS 26 全局 Chrome（Liquid Glass）突破（v2.2）** — IOS26-*（全屏 UI/UX；见 `09-BREAKTHROUGH-SPEC.md`）   (completed 2026-04-22)

## Phase Details

### Phase 1: 数据层与基线验证

**Goal**: 确认持久化层契约稳定，并建立可重复的本地验证步骤。  
...
- 01-01: 对 `bill_list` / `billRepo` 做列级与查询语义审查（可复查审计文档）  
- 01-02: 补充或更新面向开发者的数据层说明（可放在 `PROJECT.md` Context 或短 README 节）

### Phase 2: 核心用户流程 UAT

**Goal**: 主路径无阻塞缺陷，核心流程达到产品认可的完整度与连贯性。  
**Depends on**: Phase 1  
...
**Goal**: 交付半两记「预算」「资产」子模块的 **MVP 可验收** 离线能力。  
**Depends on**: Phase 2  
...
**Goal**: 在 Clay 基线上提升关键屏 **精美度**（层级、微交互、信息密度），符合半两记 UI 规格。  
**Depends on**: Phase 5  
**Requirements**: REF-04  
**UI hint**: yes  
**Success Criteria** (what must be TRUE):
...
2. 按压/列表/卡片层次有可感知的 **动效或过渡**（不要求 1:1 复刻外部模板）
...
### Phase 7: 图表 高规格表现层（v2）

**Goal**: 在 Phase 3 **图表数据逻辑**验收通过后，深化 **图表 Tab** 的表现层（柱条、切换、空态等）。  
...
2. 空态/有数据/切换周月年 的 **布局与动效** 达到产品认可的「图表精致化向」标准（以 `UI-SPEC` 为准）
3. 无未关闭的 P0 图表展示缺陷
...
- 09-01: token + `ios/` 原语（GroupedInset、ListRow、SegmentedTwo）+ typography  
- 09-02: 全局 Tab/栈导航 Chrome（已由 09 执行波次合入 `RootNavigator`）  
- 09-03 — 09-07: 逐屏与 `App` / `DESIGN` / `PROJECT` / 验收表 — 见 `09-SUMMARY.md`
